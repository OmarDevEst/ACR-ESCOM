...
def send_file(sck: socket.socket):
    ...
    filesize = os.path.getsize(f'./servidor/A2.pdf')
    # Cantidad de
    ...
    with open('./servidor/A2.pdf', "rb") as f:
        f.seek(totalBytes, 1)
    ...
    def run(self):
        while True:
            ...
            if data:
                ...
                while True:
                    if data == b'upload':
                        os.makedirs(f'servidor', exist_ok=True)
                        print("Recibiendo archivo...")
                        receive_file(self.conn, f"./servidor/A2.pdf")
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
    server.bind(('192.168.2.222', 6004))
    server.listen(2)
